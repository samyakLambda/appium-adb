...
const CONNECTION_CLOSED = new RegExp('error: closed', 'i');
...
systemCallMethods.adbExec = async function adbExec (cmd, opts) {
  ...
  const execFunc = async () => {
    try {
      let args = [...this.executable.defaultArgs, ...cmd];
      if (adbRetried) {
        let idx = args.indexOf('-s')
        let fallbackUdid = process.env.FALLBACK_UDID
        if (idx > -1 && fallbackUdid != undefined) {
          args[idx+1] = fallbackUdid
        }
      }
      ...
      const deviceconnectionClosed = CONNECTION_CLOSED.test(errText);
      if (ADB_RETRY_ERROR_PATTERNS.some((p) => p.test(errText)) || deviceconnectionClosed) {
